...
PAINT_LABELS = ("lane_left", "lane_center", "lane_right", "stop_line")  # , "crosswalk")
...
def crop_bottom(image: np.ndarray, bottom_ratio: float) -> np.ndarray:
...
def build_dataset(
    ...
    for split, split_frame_set in (("train", train_frames), ("val", val_frames)):
        ...
        for route, second in sorted(split_frame_set):
            ...
            image = crop_bottom(image, bottom_ratio)
            semantic = crop_bottom(semantic, bottom_ratio)

            stem = f"{route}__frame_{second:06d}s"
            if not cv2.imwrite(str(image_dir / f"{stem}.jpg"), image):
                raise RuntimeError(f"Could not write image for {stem}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--label", type=Path, default=Path("dataset/lane_detection_with_crossswalk/labels"))
    parser.add_argument("--output", type=Path, default=Path("dataset/lane_detection_with_crossswalk/yolo_lane_sem_class_crop"))
    ...
    parser.add_argument(
        "--bottom-ratio",
        type=float,
        default=DEFAULT_BOTTOM_RATIO,
        help="Keep only the bottom ratio of each image and mask.",
    )
