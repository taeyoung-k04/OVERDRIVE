...
def process_batch(model, frames: list[np.ndarray], imgsz: int, device: str, bottom_ratio: float) -> list[np.ndarray]:
    overlays: list[np.ndarray] = []
    for frame in frames:
        class_map = predict_bottom_class_map(model, frame, imgsz, device, bottom_ratio)
...
def render_video(
    ...
    while True:
        ...
        if len(batch) >= batch_size:
            for overlay in process_batch(model, batch, imgsz, device, bottom_ratio):
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output", type=Path, default=Path("result/lane_detection/yolo_sem_class_crop_video"))
    parser.add_argument(
        "--weights",
        type=Path,
