...
from lane_following import LaneCurve, remove_outlier_points, safe_curve_x_at
...
def _fit_points(
    ...
    residual = float(np.mean(np.abs(np.polyval(coeffs, ys) - xs)))
    point_score = min(1.0, len(points) / 20.0)
    span_score = float(np.clip(y_span / 115.0, 0.15, 1.0))
    residual_score = float(
        np.clip(1.0 - residual / max(8.0, image_width * 0.10), 0.05, 1.0)
    )
...
def fit_lane_curve_fast(
    ...
    """Fit one semantic boundary while rejecting disconnected noise.

    Compared with the previous three-line controller, this function is called
    only for yellow and green. It also limits connected-component work to each
    ...
    # One directional close and one small dilation are enough for thin lane masks.
    ...
    for label in range(1, num_labels):
        ...
        coverage_score = float(np.clip(bh / max(20.0, h * 0.45), 0.15, 1.0))
        score = curve.confidence * (0.60 + 0.40 * coverage_score)
        ...
        if previous_curve is not None:
            ...
            score *= 0.55 + 0.45 * temporal_score
    ...
    # Merge only fragments clearly aligned with the best component.
    aligned = [seed.points]
    threshold = max(10.0, w * 0.045)
    ...
    if len(aligned) > 1:
        merged = _fit_points(np.concatenate(aligned, axis=0), class_name, w)
...
class TwoLineRightFollower:
    ...
    def update(self, class_map: np.ndarray) -> TwoLineFollowingResult:
        ...
        # Blue/lane_left is intentionally never read here.
        center_curve = fit_lane_curve_fast(
            class_map,
            self.center_class_id,
            "lane_center",
            self.roi_top_ratio,
            self.previous_center_curve,
            eval_y,
            lookahead_y,
        )
        right_curve = fit_lane_curve_fast(
            class_map,
            self.right_class_id,
            "lane_right",
            self.roi_top_ratio,
            self.previous_right_curve,
            eval_y,
            lookahead_y,
        )
        ...
        detected = tuple(
            name
            for name, curve in (("center", center_curve), ("right", right_curve))
