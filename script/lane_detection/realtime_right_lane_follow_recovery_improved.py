...
Serial protocol used by the matching potentiometer-feedback Arduino sketch:
    C,<target_steering_milli>,<drive_pwm>\n
Examples:
    C,-350,90     # request a left steering position, forward PWM 90
    C,420,75      # request a right steering position, forward PWM 75
    X             # immediate stop

The Arduino closes the steering-position loop using a potentiometer.

This version accepts both telemetry formats:
    T,<sensor>,<target>,<error>,<steer_pwm>,<drive_pwm>,<actual_cmd>,<fault>\n
and the closed-loop sketch format:
    POS,raw=...,filtered=...,current=...,target=...,target_raw=...,error=...,pwm=...,fault=...

A background serial thread repeats the newest command independently of inference
FPS, while a reader thread receives actual steering-position telemetry.
"""
...
def extract_right_lane_curve(
    ...
    fragmented_fallback = False
    if component is None:
        # A semantic lane can be split into several short islands by glare or
        # worn paint. Because this mask already represents only lane_right, use
        # all plausible right-side fragments as a second chance.
        component = mask.copy()
        component[:, : int(width * 0.30)] = 0
        if int(np.count_nonzero(component)) < max(12, min_component_area // 2):
            return RightLaneObservation(valid=False, mask=mask, reason="no right-lane component")
        fragmented_fallback = True
    ...
    if len(row_points) < min_lane_points and not fragmented_fallback:
        # Retry with every cleaned right-lane fragment. This helps when the
        # correct marking is broken into components that are individually too
        # small for the primary selector.
        fallback_component = mask.copy()
        fallback_component[:, : int(width * 0.30)] = 0
        fallback_points: list[tuple[float, float]] = []
        for y in range(roi_top, height, 2):
            xs = np.flatnonzero(fallback_component[y])
            if xs.size == 0:
                continue
            fallback_points.append((float(np.median(xs)), float(y)))
        if len(fallback_points) > len(row_points):
            component = fallback_component
            row_points = fallback_points
            fragmented_fallback = True
    ...
    return RightLaneObservation(
        ...
        confidence=confidence,
        residual_px=residual_px,
        reason="fragmented fallback" if fragmented_fallback else "ok",
    )
...
class RightLaneFollower:
    ...
    def reset(self) -> None:
        ...
        self.previous_steering = 0.0
        self.lost_frames = 0
        self.smoothed_coefficients = None
        self.last_valid_coefficients = None
        self.last_valid_confidence = 0.0
    ...
    def _compute_from_coefficients(
        ...
        return ControlOutput(
            ...
            reason="predicted from last valid lane" if predicted else "ok",
        )
    ...
    def _lost_output(
        ...
        if lane_loss_age <= grace:
            ...
            output.reason = f"coasting on last lane ({observation.reason})"
            return output
        ...
        return ControlOutput(
            ...
            lost_frames=self.lost_frames,
            lane_loss_age=lane_loss_age,
            reason=f"lane lost for {lane_loss_age:.2f}s: safety stop ({observation.reason})",
        )
    ...
    def compute(
        ...
        if (
            ...
            return self._lost_output(observation, frame_shape)
        ...
        if coefficients is None:
            rejected = RightLaneObservation(
                valid=False,
                confidence=observation.confidence,
                reason="sudden lane jump rejected",
            )
            return self._lost_output(rejected, frame_shape)
        ...
        self.last_valid_coefficients = coefficients.copy()
        self.last_valid_confidence = observation.confidence
        self.last_valid_time = now
        self.last_valid_curvature = output.curvature
        self.last_valid_combined_error = output.combined_error
        return output
...
def main() -> None:
    ...
    print("Controls: SPACE=start/stop, S=emergency stop, R=reset Arduino fault, Q or ESC=quit", flush=True)
